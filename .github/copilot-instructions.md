...
## General Guidelines
- **Read Specifications First**: Before implementing any core logic, look for relevant BDD specs in `05-Specification-Driven-Development/` (e.g., `GAME_CHANGER_FEATURES_SPECS.md`). Do not hallucinate behavior that contradicts these specs.
- **Adhere to Architecture**: Understand that this project uses a 5-layer resilient architecture loop. Do not simplify the loop into a basic `while` loop. If you are modifying the loop, reference `04-Architecture/ITERATION_LOOP_DESIGN.md`.
...
- **Testing (Phase 6)**: When asked to write tests, always use the skeletons provided in `07-Testing/TEST_SKELETON_GUIDE.md` (xUnit for C#, Expecto for F#).
